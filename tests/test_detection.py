from src.schemas.session_features import SESSION_FEATURES_SCHEMA
...
def test_rule_based_detection_flags_high_risk_session(spark, session_feature_factory):
    ...
    df = spark.createDataFrame(rows, schema=SESSION_FEATURES_SCHEMA)
    out = build_rule_based_detection(df)

    row = out.collect()[0]

    assert row["rule_score"] >= 4
